...
log_file="hadoop_install_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -i $log_file)
exec 2>&1
...
echo -e "${GREEN}📥 Downloading and extracting Hadoop...${NC}"
HADOOP_VERSION=3.4.0
HADOOP_HOME=$HOME/hadoop-$HADOOP_VERSION

if [ -d "$HADOOP_HOME" ]; then
    echo "🔍 Hadoop already exists. Skipping..."
else
...
echo -e "${GREEN}📁 Creating temp and data directories...${NC}"
